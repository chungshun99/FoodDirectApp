...
const ViewAllDonationList = ({route, navigation}) => {
  ...
  const getTempDonationHistory = async () => {
    ...
    .then(collectionSnapshot => {
      ...
      collectionSnapshot.forEach(donationRequestSnapshot => {
        ...
        var requestdate = new Date(donationRequestSnapshot.data().donationRequestTime.seconds * 1000 + donationRequestSnapshot.data().donationRequestTime.nanoseconds/1000000);
        let formatted_requestdate = requestdate.getDate() + " " + months[requestdate.getMonth()] + " " + requestdate.getFullYear();  
        
        var requesteddatetext = formatted_requestdate.toString();
        
        donationList.push({ ...donationRequestSnapshot.data(), donationAcceptdate: datetext, donationRequestdate: requesteddatetext, id: donationRequestSnapshot.id });
  ...
  return (
    ...
    >
      {historyList.map((history) => {
        return (
          <TouchableOpacity key={history.id} onPress={() => navigation.navigate("View Donation Details", {donationId: history.id})}>
            <View
            ...
            >
              <View style={styles.listRow}>
                <View style={styles.details}>
                  ...
                  {history.donationState !== 'Available' &&
                  <View style={styles.iconRow}>
...
export default ViewAllDonationList
